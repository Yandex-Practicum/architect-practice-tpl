...
|---|---|---|---|---|---|---|
|Сроки|r-001|MVP необходимо сделать в течение месяца|сроки|заказчик|утвержденный|выход сотрудников через 4 недели.|
|Исполнители|r-002|Реализовать необходимо своими силами текущих команд, без дополнительного найма|ресурсы|заказчик|новый|необходимо согласовать штат исполнителей, и желательно обойтись без найма|
|Железо|r-003|Ничего не покупать для старта проекта|технологические|заказчик|новый|выделить одну виртуалку на тест среду, одну на прод (никаких кластеров)|
|Мобильные платформы|r-004|Не делать мобильные приложения|технологические|заказчик|в процессе|выделить одну виртуалку на тест среду, одну на прод (никаких кластеров)|
...
| - | - | - | - | - | - | - | - |
|Кабинет сотрудника|func-001|Аутентификация через веб-интерфейс|Конечные пользователи|Обязательно|отдел ИБ/поддержка|Петр Иванов|Нужно простая удобная веб-форма аутентификации|
|Кабинет сотрудника|func-002|Список доступных мест с фильтрацией на дату|Конечные пользователи|Обязательно|заказчик|Иван Петров|Минималистично - учить новый css фреймворк необязательно|
|Кабинет сотрудника|func-003|Форма для создания/редактирования/отмены парковочного места|Конечные пользователи|Обязательно|заказчик|Иван Петров||
|Кабинет сотрудника|func-004|Страница отчета по забронированным сотрудником местам с фильтрами по датам с оставшимся количеством по квоте на текущий момент |Конечные пользователи|Обязательно|заказчик|Иван Петров||
|Кабинет офис-менеджера|func-005|Аутентификация через веб-интерфейс|офис-менеджеры|Обязательно|отдел ИБ/поддержка|Петр Иванов|Нужно простая удобная веб-форма аутентификации. Отличается от кабинета сотрудника. согласовать с ИБ|
|Кабинет офис-менеджера|func-051|Форма регистрации по индивидуальному коду |отдел ИБ|Обязательно|отдел ИБ|||
|Кабинет офис-менеджера|func-006|Список сотрудников с возможностью фильтрации по имени |офис-менеджеры|Обязательно|заказчик/офис-менеджеры|Петр Иванов|Обычный CRUD|
|Кабинет офис-менеджера|func-007|Форма редактирования сотрудника с возможностью создания/редактирования/удаления, а так же установления лимитов |офис-менеджеры|Обязательно|заказчик/офис-менеджеры|Петр Иванов|Обычный CRUD|
|Кабинет офис-менеджера|func-008|Список парковочных мест с возможностью создания/редактирования/удаления|офис-менеджеры|Обязательно|заказчик/офис-менеджеры|Петр Иванов|Обычный CRUD|
|Кабинет офис-менеджера|func-009|Список шаблонов системных уведомлений с возможностью создания/редактирования/удаления|офис-менеджеры|Обязательно|отдел ИБ/поддержка|Петр Иванов||
...
|---|---|---|---|---|---|---|---|
|Безопасность|un-001|Для работы в системе необходима аутентификация|Обязательно|Отдел ИБ||в процессе|Для всех сотрудников доступ по логину|
|Безопасность|un-002|Регистрация возможна только для офис-менеджеров |Обязательно|Отдел ИБ||в процессе||
|Масштабируемость|un-003|Пилотный проект запускаем на одном сервере максимально экономно|Обязательно|Отдел разработки||в процессе|docker-compose нам в помощь|
|восстанавливаемость|un-004|Настроить автоматический бэкап базы данных раз в день  с отправкой в облко|Обязательно|Отдел разработки||в процессе||
|интеграция|un-005|развернуть один инстанс Postgres в качестве хранения данных|Обязательно|Отдел разработки||в процессе||
|интеграция|un-006|сервис нотификации подключить сторонний|Обязательно|Отдел разработки||в процессе|свой не разрабатываем. подключаемся к стороннему: 1 день работы программиста|
---
