...
Архитектор: Роман Андреев

Дата: 25 Сентября 2023

| Дата | Автор | Что изменилось |
| :-: | :-: | :-: |
| 25.09.2023| Роман Андреев | Все |

## 1. Ограничения

|Тип|ID|Описание|Группа источника ограничения|Источник ограничения|Статус|Комментарий|
|---|---|---|---|---|---|---|
|Сроки|С-001|Завершить проект до 15.10.2023|Конечные пользователи|Заказчик|Утвержденный||
|Ресурсы|С-002|Пять человеко-часов в неделю|Конечные пользователи|Заказчик|Утвержденный||

## 2. Функциональные требования


| ID | Описание | Приоритет | Группа источника требований | Источник требований | Статус | Комментарий |
| - | - | - | - | - | - | - |
|FR-001|В системе должны быть доступны следующие роли пользователей: "Офис-Менеджер" и "Сотрудник"|Обязательно|Конечные пользователи|Заказчик|Утвержденный||
|FR-002|Система должна позволять зарегистрировать пользователя с ролью "Офис-Менеджер" по секретному коду|Обязательно|Конечные пользователи|Заказчик|Утвержденный||
|FR-003|Пользователь с ролью "Офис-Менеджер" должен иметь возможность добавлять и удалять пользователей с ролью "Сотрудник"|Обязательно|Конечные пользователи|Заказчик|Утвержденный||
|FR-004|Пользователь с ролью "Офис-Менеджер" должен иметь возможность добавлять и удалять парковочные места|Обязательно|Конечные пользователи|Заказчик|Утвержденный||
|FR-005|Пользователь с ролью "Офис-Менеджер" должен иметь возможность устанавливать лимиты на количество бронирований в месяц для сотрудников|Обязательно|Конечные пользователи|Заказчик|Утвержденный||
|FR-006|Пользователь с ролью "Офис-Менеджер" должен иметь возможность устанавливать время отправки, шаблона письма и адрес получателя для автоматической повторяющейся отправки информации о бронированиях|Обязательно|Конечные пользователи|Заказчик|Утвержденный||
|FR-007|Система должна ежедневно автоматически отправлять электронное письмо на заданный адрес в установленное время.|Обязательно|Конечные пользователи|Заказчик|Утвержденный|Письмо содержит список регистрационных номеров автомобилей из списка бронирований на следующие сутки|
|FR-008|Пользователь с ролью "Сотрудник" должен иметь доступ к системе с помощью предоставленного логина.|Обязательно|Конечные пользователи|Заказчик|Утвержденный|Логин добавляется в систему пользователем с ролью "Офис-Менеджер"|
|FR-009|Пользователь с ролью "Сотрудник" должен иметь возможность запросить доступные для бронирования места в выбранную дату|Обязательно|Конечные пользователи|Заказчик|Утвержденный||
|FR-010|Пользователь с ролью "Сотрудник" должен иметь возможность забронировать выбранное пользователем свободное место в заданную дату|Обязательно|Конечные пользователи|Заказчик|Утвержденный||
|FR-011|Пользователь с ролью "Сотрудник" должен иметь возможность отменить бронь места|Обязательно|Конечные пользователи|Заказчик|Утвержденный|При отмене брони в день записи с баланса сотрудника должен списываться 1 день|
|FR-012|Пользователь с ролью "Сотрудник" должен иметь возможность посмотреть историю своих бронирований|Обязательно|Конечные пользователи|Заказчик|Утвержденный||
|FR-013|Пользователь с ролью "Сотрудник" должен иметь возможность посмотреть количество доступных для бронирования дней в текущем месяце|Обязательно|Конечные пользователи|Заказчик|Утвержденный||
...
|---|---|---|---|---|---|---|---|
|Безопасность|FR-001|Отсуствие уязвимостей из списка OWASP Top 10|Желательно|Безопасность|Заказчик|Утвержденный||
|Надежность|FR-002|Хранение данных в базе данных|Обязательно|Техническая команда|Заказчик|Утвержденный||
|Доступность|FR-003|Сервис должен быть доступен 99% времени|Обязательно|Конечные пользователи|Заказчик|Утвержденный||
|Производительность|FR-004|Севис должен быть рассчитан на нагрузку до 100 rps|Обязательно|Техническая команда|Заказчик|Утвержденный||
|Масштабируемость|FR-005|В случае успеха проекта должна быть предусмотренна возможность адаптировать сервис к более высоким нагрузкам|Возможно|Техническая команда|Заказчик|Утвержденный||
|Интеграция|FR-006|Предусмотреть возможность интеграции со сторонним сервисом аутентификации|Желательно|Техническая команда|Заказчик|Утвержденный||
