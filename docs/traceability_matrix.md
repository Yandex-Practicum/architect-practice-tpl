...
|Границы проекта| C-007 | Проект должен иметь возможность масштабирования на несколько офисов | Партнеры третьих сторон |Заказчик||||
...
|Границы проекта| С-009 | Система должна работать круглосуточно |Операционные команды/Команды поддержки |Заказчик||| Сбой системы не приведёт к высоким потерям, так как остаётся возможность ручного управления бронированием с использованием легаси-системы с некоторым увеличением нагрузки на офис-менеджера |
...
| - | - | - | - | - | - | - | - |
| Аутентификация | FR-001 | В системе должен быть реализован доступ сотрудника по логину (добавляется офис-менеджером в систему). | Обязательно | Конечные пользователи | Сотрудник | Новый ||
| Бронирование | FR-002 | Сотруднику должен быть доступен запрос списка доступных для бронирования мест на выбранную дату вместе со схемой парковки. | Обязательно | Конечные пользователи | Сотрудник | Новый ||
| Бронирование | FR-003 | Сотрудник должен иметь возможность просмотра, создания, переноса и отмены бронирований парковочных мест на корпоративной парковке. | Обязательно | Конечные пользователи | Сотрудник | Новый ||
| Бронирование | FR-004 | Система должна осуществлять учёт и ведение бронирований с выводом остатка дней в текущем месяце | Обязательно | Конечные пользователи | Сотрудник | Новый ||
| Администрирование | FR-005 | Регистрация офис-менеджера должна осуществляться по секретному коду. | Обязательно | Операционные команды/Команды поддержки | Офис-менеджер | Новый ||
| Администрирование | FR-006 | Система должна обеспечить добавление и удаление логинов сотрудников в систему офис-менеджером | Обязательно |  Операционные команды/Команды поддержки | Офис-менеджер | Новый ||
| Администрирование | FR-007 | В системе должно быть реализовано добавление и удаление схемы парковки с кодовыми обозначениями парковочных мест офис-менеджером| Обязательно |  Операционные команды/Команды поддержки | Офис-менеджер | Новый ||
| Администрирование | FR-008 | В системе должно быть реализовано добавление и удаление парковочных мест офис-менеджером| Обязательно |  Операционные команды/Команды поддержки | Офис-менеджер | Новый ||
| Администрирование | FR-009 | В системе должна быть реализована установка лимитов на количество бронирований в месяц для сотрудников | Обязательно |  Операционные команды/Команды поддержки | Офис-менеджер | Новый ||
| Администрирование | FR-010 | В системе должно быть реализована установка времени, шаблона письма и адреса получателя для автоматической повторяющейся отправки информации о бронированиях | Обязательно |  Операционные команды/Команды поддержки | Офис-менеджер | Новый ||
...
|---|---|---|---|---|---|---|---|
| Гибкость ||Система должна иметь возможность расширения для возможности обслуживания всех арендаторов здания||Партнеры третьих сторон|Владельцы офиса|Новый| |
| Удобство использования || Система должна иметь минимально удобную автоматизацию ||Партнеры третьих сторон|Владельцы офиса|Новый| |
| Интеграция ||Система должна иметь возможность интеграции с внешней системой авторизации|||Заказчик|Новый||
|Удобство использования||Интерфейс пользователя должен быть реализован в виде веб-сайта с минимально необходимым функционалом|||Заказчик|Новый||
