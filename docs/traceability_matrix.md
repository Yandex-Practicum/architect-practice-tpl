...
| - | - | - | - | - | - | - | - |
| Аутентификация | - | В системе должен быть реализован доступ сотрудника по логину (добавляется офис-менеджером в систему). | Обязательно | Конечные пользователи | Сотрудник | Новый ||
| - | - | Сотруднику должен быть доступен запрос списка доступных для бронирования мест на выбранную дату вместе со схемой парковки. | Обязательно | Конечные пользователи | Сотрудник | Новый ||
| - | - | Сотрудник должен иметь возможность просмотра, создания, переноса и отмены бронирований парковочных мест на корпоративной парковке. | Обязательно | Конечные пользователи | Сотрудник | Новый ||
| - | - | Система должна осуществлять учёт и ведение бронирований с выводом остатка дней в текущем месяце | Обязательно | Конечные пользователи | Сотрудник | Новый ||
| - | - | Регистрация офис-менеджера должна осуществляться по секретному коду; | Обязательно | Конечные пользователи | Офис-менеджер | Новый ||
| - | - | Система должна обеспечить добавление и удаление логинов сотрудников в систему офис-менеджером | Обязательно |  Операционные команды/Команды поддержки | Офис-менеджер | Новый ||
| - | - | В системе должно быть реализовано добавление и удаление схемы парковки с кодовыми обозначениями парковочных мест офис-менеджером| Обязательно |  Операционные команды/Команды поддержки | Офис-менеджер | Новый ||
| - | - | В системе должно быть реализовано добавление и удаление парковочных мест офис-менеджером| Обязательно |  Операционные команды/Команды поддержки | Офис-менеджер | Новый ||
| - | - | В системе должна быть реализована установка лимитов на количество бронирований в месяц для сотрудников | Обязательно |  Операционные команды/Команды поддержки | Офис-менеджер | Новый ||
| - | - | В системе должно быть реализована установка времени, шаблона письма и адреса получателя для автоматической повторяющейся отправки информации о бронированиях | Обязательно |  Операционные команды/Команды поддержки | Офис-менеджер | Новый ||
| - | - | Система должна реализовать функционал автоматической повторяющейся отправки электронного письма на заданный адрес, в котором содержится список регистрационных номеров автомобилей из списка бронирований на следующие сутки. После отправки сообщения, операции с бронированием на следующие сутки не допускаются. | Обязательно |  Партнеры третьих сторон | Охрана парковки | Новый ||
